...
from myoutreach.core.agents.follow_up import FollowUpDecision
from myoutreach.core.db.deals import set_profile_state
from myoutreach.linkedin.db.leads import create_enriched_lead, promote_lead_to_deal
from myoutreach.core.models import Task
...
from myoutreach.linkedin.tasks.connect import ConnectStrategy, handle_connect
from myoutreach.linkedin.tasks.check_pending import handle_check_pending
from myoutreach.linkedin.tasks.follow_up import handle_follow_up
...
class TestHandleConnect:
    ...
    @patch("myoutreach.linkedin.tasks.connect.strategy_for")
    ...
    def test_pending_stamps_next_check(self, mock_status, mock_send, mock_visit, mock_strategy, fake_session):
        """Connect → FAILED: connection disabled, next_check_pending_at stays None."""
        _make_qualified(fake_session)
        mock_strategy.return_value = _mock_strategy(self._candidate())
        mock_status.return_value = ProfileState.QUALIFIED
        mock_send.return_value = ProfileState.PENDING
...
class TestHandleFollowUp:
    @patch("myoutreach.linkedin.db.chat.sync_conversation")
    @patch("myoutreach.core.db.summaries.materialize_profile_summary_if_missing")
    @patch("linkedin_cli.actions.message.send_raw_message", return_value=True)
    @patch("myoutreach.core.agents.follow_up.run_follow_up_agent")
    ...
    def test_wait_bumps_update_date(self, mock_agent, mock_materialize, fake_session):
        ...
        # Shift the original update date into the past to prevent flakiness
        deal_before.update_date = timezone.now() - timedelta(seconds=5)
