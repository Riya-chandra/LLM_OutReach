...
def _mock_embeddings(request):
    ...
    if "no_embed_mock" in request.keywords:
        ...
        with patch("myoutreach.linkedin.ml.embeddings.embed_text", return_value=np.ones(384)):
            yield
...
def _mock_contact_capture(request):
    ...
    if "no_contact_capture_mock" in request.keywords:
        ...
        with patch("myoutreach.crm.models.lead.Lead.capture_contact_info", return_value=None):
            yield
