...
from myoutreach.linkedin.pipeline.qualify import run_qualification
...
def _create_lead_with_embedding(lead_id, public_id):
    from myoutreach.crm.models import Lead
...
class TestQualifyAutoDecisions:
    def test_always_calls_llm(self, db):
        ...
        with (
            patch("myoutreach.linkedin.db.leads.get_leads_for_qualification", return_value=_fake_leads()),
            patch("myoutreach.linkedin.pipeline.qualify._fetch_profile_text", return_value="engineer at acme"),
            patch("myoutreach.linkedin.ml.qualifier.qualify_with_llm", return_value=(1, "Good fit")) as mock_llm,
            patch.object(qualifier, "update"),
            patch("myoutreach.linkedin.db.leads.promote_lead_to_deal"),
        ):
    ...
    def test_llm_on_cold_start(self, db):
        ...
        with (
            patch("myoutreach.linkedin.db.leads.get_leads_for_qualification", return_value=_fake_leads()),
            patch("myoutreach.linkedin.pipeline.qualify._fetch_profile_text", return_value="engineer at acme"),
            patch("myoutreach.linkedin.ml.qualifier.qualify_with_llm", return_value=(0, "Bad fit")) as mock_llm,
            patch.object(qualifier, "update"),
            patch("myoutreach.core.db.deals.create_disqualified_deal"),
        ):
            run_qualification(session, qualifier)
            mock_llm.assert_called_once()
    ...
    def test_disqualify_on_promote_failure(self, db):
        ...
        with (
            patch("myoutreach.linkedin.db.leads.get_leads_for_qualification", return_value=_fake_leads()),
            patch("myoutreach.linkedin.pipeline.qualify._fetch_profile_text", return_value="engineer at acme"),
            patch("myoutreach.linkedin.ml.qualifier.qualify_with_llm", return_value=(1, "Good fit")),
            patch.object(qualifier, "update"),
            patch("myoutreach.linkedin.db.leads.promote_lead_to_deal",
                  side_effect=ValueError("no company_name")),
            patch("myoutreach.core.db.deals.create_disqualified_deal") as mock_disqualify,
        ):
            run_qualification(session, qualifier)
