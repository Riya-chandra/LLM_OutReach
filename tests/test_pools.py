...
from myoutreach.linkedin.ml.qualifier import BayesianQualifier
from myoutreach.linkedin.pipeline.pools import (
    find_candidate,
    _needs_search,
    search_source,
    qualify_source,
...
class TestSearchSource:
    def test_yields_keywords(self):
        with patch("myoutreach.linkedin.pipeline.pools.run_search", side_effect=["kw1", "kw2", None]):
            results = list(search_source("session"))
        assert results == ["kw1", "kw2"]

    def test_stops_on_none(self):
        with patch("myoutreach.linkedin.pipeline.pools.run_search", return_value=None):
            results = list(search_source("session"))
        assert results == []
...
class TestQualifySource:
    def test_qualifies_without_search_when_pool_ok(self):
        ...
        with (
            patch("myoutreach.linkedin.pipeline.pools.fetch_qualification_candidates", return_value=candidates),
            patch("myoutreach.linkedin.pipeline.pools._needs_search", return_value=False),
            patch("myoutreach.linkedin.pipeline.pools.run_qualification", side_effect=["alice", None]),
            patch("myoutreach.linkedin.pipeline.pools.run_search") as mock_search,
        ):
    ...
    def test_searches_until_exhausted_when_pool_empty_exploit(self):
        ...
        with (
            ...
            patch("myoutreach.linkedin.pipeline.pools._needs_search", return_value=True),
            patch("myoutreach.linkedin.pipeline.pools.run_qualification", side_effect=["alice", None]),
            patch("myoutreach.linkedin.pipeline.pools.run_search", side_effect=["kw1", "kw2", None]) as mock_search,
        ):
            results = list(qualify_source("session", scorer))
    ...
    def test_search_stops_when_pool_fills(self):
        ...
        with (
            patch("myoutreach.linkedin.pipeline.pools.fetch_qualification_candidates", return_value=candidates),
            patch("myoutreach.linkedin.pipeline.pools._needs_search", side_effect=pool_empty_side_effect),
            patch("myoutreach.linkedin.pipeline.pools.run_qualification", side_effect=["alice", None]),
            patch("myoutreach.linkedin.pipeline.pools.run_search", return_value="kw") as mock_search,
        ):
            results = list(qualify_source("session", scorer))
    ...
    def test_searches_when_no_candidates(self):
        ...
        with (
            patch("myoutreach.linkedin.pipeline.pools.fetch_qualification_candidates",
                  side_effect=[[], candidates, candidates]),
            patch("myoutreach.linkedin.pipeline.pools._needs_search", return_value=False),
            patch("myoutreach.linkedin.pipeline.pools.run_qualification", side_effect=["alice", None]),
            patch("myoutreach.linkedin.pipeline.pools.run_search", return_value="kw1") as mock_search,
        ):
            results = list(qualify_source("session", scorer))
    ...
    def test_stops_when_search_exhausted_and_no_candidates(self):
        ...
        with (
            patch("myoutreach.linkedin.pipeline.pools.fetch_qualification_candidates", return_value=[]),
            patch("myoutreach.linkedin.pipeline.pools.run_search", return_value=None),
            patch("myoutreach.linkedin.pipeline.pools.run_qualification") as mock_qualify,
        ):
            results = list(qualify_source("session", scorer))
...
class TestGetCandidate:
    ...
    def test_backfills_then_returns(self, fake_session):
        ...
        with (
            patch("myoutreach.linkedin.pipeline.pools.find_ready_candidate", side_effect=[None, candidate]),
            patch("myoutreach.linkedin.pipeline.pools.promote_to_ready", side_effect=[0, 1]),
            patch("myoutreach.linkedin.pipeline.pools.fetch_qualification_candidates", return_value=candidates),
            patch("myoutreach.linkedin.pipeline.pools._needs_search", return_value=False),
            patch("myoutreach.linkedin.pipeline.pools.run_qualification", return_value="alice"),
        ):
    ...
    def test_exhausted_returns_none(self, fake_session):
        ...
        with (
            patch("myoutreach.linkedin.pipeline.pools.find_ready_candidate", return_value=None),
            patch("myoutreach.linkedin.pipeline.pools.promote_to_ready", return_value=0),
            patch("myoutreach.linkedin.pipeline.pools.fetch_qualification_candidates", return_value=[]),
            patch("myoutreach.linkedin.pipeline.pools.run_search", return_value=None),
        ):
