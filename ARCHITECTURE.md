...
Detailed module documentation for myoutreach. See `CLAUDE.md` for rules and quick reference.

## Project Layout

All source lives in the single `myoutreach/` package; Django apps are nested inside it
...
- `collect_from_wizard()` — interactive questionary wizard (needs TTY), only asks for `missing_keys()`. Backed by the vendored `onboarding_wizard.py` (step engine) + `onboarding_prompts.py` (`SELF_HOSTED_QUESTIONS`) — no external `myoutreach` dependency.
...
Task rows are **lazy**: `payload = {"campaign_id": <id>}` only — no `public_id`, no deal reference. The handler resolves a concrete target at execution time via a single eligibility query. Slot creation is centralized in `myoutreach/core/scheduler.py`; no other module inserts Task rows. The module is organized in three layers:
...
Three task types (handlers in `myoutreach/linkedin/tasks/`, signature: `handle_*(task, session, qualifiers)`):
...
Five apps in `INSTALLED_APPS`, all nested under the `myoutreach/` package (see Project Layout):
...
Paths below are relative to `myoutreach/`.
...
- **`linkedin/diagnostics.py`** — `failure_diagnostics()` context manager, `capture_failure()` saves page HTML/screenshot/traceback to `/tmp/myoutreach-diagnostics/`.
...
- **`auth.py`** — the auth flow, declared as `@auth_flow.transition` actions
  ...
  *, username=, password=)` stamps credentials and runs the flow to the feed. Shared by
  the CLI `login` verb and the daemon (`myoutreach/linkedin/browser/launch.py`), so both drive one
...
- **`linkedin/setup/self_profile.py`** — `discover_self_profile(session)`: Voyager `me`
  scrape → dict, no persistence (the disqualified-lead write is myoutreach's
...
- **Prompt templates** (at `myoutreach/core/templates/prompts/`) — `qualify_lead.j2` (temp 0.7), `search_keywords.j2` (temp 0.9), `follow_up_agent.j2`.
...
- `deploy.yml` — Tests → build + push to `ghcr.io/eracle/myoutreach`. Tags: `latest`, `sha-<commit>`, semver.
