...
```
myoutreach campaign logic (when/who/why)
        |
...
## What stays in myoutreach

| Concern | Notes |
|---|---|
| Campaign logic | Task queue, state machine, scheduling |
| Django models | Lead, Deal, Campaign, LinkedInProfile |
| Passive profile discovery | `_discover_and_enrich()` from nav.py — myoutreach hooks into navigation events, not the library |
...
1. ~~Extract in-place: cut all Django/DB imports from library modules, verify myoutreach still works~~ **Done** — `url_utils` moved out of `db/`, `VOYAGER_REQUEST_TIMEOUT_MS` moved to `api/client.py`, `crm.models` removed from actions CLI
...
7. Publish as `playwright-linkedin`, update myoutreach to `pip install` it
