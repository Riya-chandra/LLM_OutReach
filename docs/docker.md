...
```bash
docker run --pull always -it -p 6080:6080 -p 5900:5900 -v myoutreach_db:/app/data ghcr.io/eracle/myoutreach:latest
```

Watch the live browser (and clear any LinkedIn checkpoint) at **http://localhost:6080/vnc.html**.

The interactive onboarding will guide you through LinkedIn credentials, LLM API key, and campaign setup on first run. All data (CRM database, cookies, model blobs, embeddings) persists in the `myoutreach_db` Docker volume.
...
# Restart (data persists in the myoutreach_db volume)
docker run --pull always -it -p 6080:6080 -p 5900:5900 -v myoutreach_db:/app/data ghcr.io/eracle/myoutreach:latest
```
...
The pre-built `docker run` command uses a named Docker volume (`myoutreach_db`) mounted at `/app/data` for data persistence (database, config). The compose setup (`local.yml`) mounts the entire repo `.:/app` for live code editing during development.
