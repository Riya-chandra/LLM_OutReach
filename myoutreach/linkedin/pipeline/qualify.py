# myoutreach/linkedin/pipeline/qualify.py
...
def fetch_qualification_candidates(session):
    ...
    from myoutreach.linkedin.db.leads import get_leads_for_qualification
...
def run_qualification(session, qualifier: BayesianQualifier) -> str | None:
    ...
    from myoutreach.linkedin.ml.qualifier import qualify_with_llm, format_prediction
...
def _save_qualification_result(session, qualifier: BayesianQualifier, lead_id: int, public_id: str, embedding: np.ndarray, label: int, reason: str):
    ...
    from myoutreach.core.db.deals import create_disqualified_deal
    from myoutreach.linkedin.db.leads import promote_lead_to_deal
...
def _fetch_profile_text(session, lead_id: int, public_id: str) -> str | None:
    from myoutreach.crm.models import Lead
    from myoutreach.linkedin.ml.profile_text import build_profile_text
